...
def init_jaeger_client(key, default_value):
    ...
    tracer = jaeger_client.Config(
        config={
            ...
            "logging": get_bool_from_env(key, default_value),
        },
        service_name="saleor",
        validate=True,
    ).initialize_tracer()
...
INIT_JAEGER_CLIENT = init_jaeger_client

if "JAEGER_AGENT_HOST" in os.environ and "UWSGI" not in os.environ:
    INIT_JAEGER_CLIENT("JAEGER_LOGGING", False)
